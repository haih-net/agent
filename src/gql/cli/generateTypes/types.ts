...
/**
 * Function for getting unique array elements
...
/** Function for getting files using glob */
...
/** Function for getting list of query file paths from frontend */
...
/** Function creating Map with graphql files */
...
/**
 * Function generating index.ts from Map with graphql files
 */
async function generateTypesFromMap() {
  const filesMap = await createInitialMap()

  /**
   * Do nothing if no files were received
   */
  if (!filesMap.size) {
    return
  }
...
/** Function creating index.ts */
async function generateTypes() {
  // Clear directory from previously generated ts-files
  await clearOutputDirectory()

  // Generate types from API schema
  await generateTypesFromSchema()

  // Generate types for frontend queries
